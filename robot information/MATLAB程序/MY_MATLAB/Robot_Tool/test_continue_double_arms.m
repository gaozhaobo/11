...
test_ql_0 =[-0.1198723   1.146691-pi/2   0.2785613 0 -1.4252519  -0.1198723];

test_ql_end = [-0.659254742830952  -0.130556422245043  -0.987061790471323   0.000000639598099  -0.453177830369662  -0.909255314390695];
% T_start = [0.0143   -0.1187   -0.9928       402
%     0.1187   -0.9857    0.1196    -48.42
%    -0.9928   -0.1196   -0.0000    -128.5
%          0         0         0         1];
...
test_qf_0 = [pi/2,pi/2-pi/2,pi/2-pi/2,-pi/2+pi/2,pi/2,-pi/2];
test_V_end = [-2.5,-1.5,-2.5,0,0,0.0025]';
...
for i=t
    ...
    [V_f,V_l] = Caculate_Robot2_V_with_Robot1(test_ql_next,test_qf_next,[0,0,0],[0,0,0],test_V_end,[0,500,0]);%求得时刻主从臂关节速度
...
figure;  % 打开一个新图形窗口
plot(points, 500-L_error, 'b-', 'LineWidth', 2);  % 绘制 B 值的线

% 4. 添加标签和标题
xlabel('时间,单位0.1s');  % 横坐标标签
ylabel('值');    % 纵坐标标签
title('夹具末端相对距离误差，单位mm');  % 图形标题
grid on;        % 显示网格
