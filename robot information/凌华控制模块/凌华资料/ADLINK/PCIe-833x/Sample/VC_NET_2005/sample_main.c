...
void get_version_info( I32 *Array_size_5, I32 Board_ID )
{
  ...
  // Get Driver version
  APS_get_device_info( Board_ID, 0x10, &Array_size_5[IDX_DRIVER] );
  
  // Get DSP kernel version
  APS_get_device_info( Board_ID, 0x40, &Array_size_5[IDX_KERNEL] ); 
  
  // Get FPGA version
  APS_get_device_info( Board_ID, 0x21, &Array_size_5[IDX_FIRMWARE] );

  // Get PCB version
  APS_get_device_info( Board_ID, 0x30, &Array_size_5[IDX_PCB] );
}
...
void home_move_example( I32 Axis_ID )
{
  ...
  APS_set_axis_param( axis_id, PRA_HOME_DIR,       1 ); // Set home direction , negative direction.
  ...
  APS_set_axis_param( axis_id, PRA_HOME_SHIFT,     0 ); // Set home position shift distance. 
...
{
  I32 axis_id = Axis_ID;
  F64 speed_1 =  6000000.0;
  F64 speed_2 = 10000.0;
  F64 speed_3 =  6000.0;
  
  APS_set_axis_param_f( axis_id, PRA_STP_DEC, 10000.0 );
  APS_set_axis_param_f( axis_id, PRA_CURVE,      0.5 ); //Set acceleration rate
  APS_set_axis_param_f( axis_id, PRA_ACC,    1000000.0 ); //Set acceleration rate
  APS_set_axis_param_f( axis_id, PRA_DEC,    1000000.0 ); //Set deceleration rate
...
{
	APS_set_axis_param_f( Axis_ID, PRA_STP_DEC, 100000.0 );
...
void jog_move_via_DI_example( I32 Axis_ID )
{
	...
	APS_set_axis_param( Axis_ID, PRA_JG_MAP_DI_EN, 0x3 ); // Enable positive DI and negative DI
...
void p2p_example( I32 Axis_ID )
{
	...
	ret = APS_ptp( Axis_ID, OPT_RELATIVE, 10000.0 , 0 );
...
void interpolation_3D_arc_example( I32 *Axis_ID_Array )
{
	...
	F64 CenterArray[3] = { 10000.0 , 10000.0, 0 };
	F64 EndArray[3]    = { 10000.0 , 10000.0, 10000.0 * sqrt( 2 ) };
	I16 Dir = 0;
	F64 TransPara = 0;
	int i;
...
void DIO_sample( I32 Board_ID )
{
	...
	ret = APS_get_field_bus_d_channel_output(Board_ID,Bus_No,Mod_No,Ch_No,&DO_Value);
	//*******************************************************************
	DO_Value = 1;
	ret = APS_set_field_bus_d_channel_output(Board_ID,Bus_No,Mod_No,Ch_No,DO_Value);
...
void AIO_sample( I32 Board_ID )
{
	...
	ret = APS_set_field_bus_a_output(  Board_ID,  BUS_No,  MOD_No,  Ch_No,   AO_Value );
    ret = APS_get_field_bus_a_input(  Board_ID,  BUS_No,  MOD_No,  Ch_No, &AI_Value );
...
void gear_example( I32 *Axis_ID_Array )
{
	...
	if( status == 0 )
	{
		APS_set_axis_param( slaveAxis, PRA_GEAR_MASTER, 0 ); // Select axis 0 to be axis 1's master
		
		//APS_set_axis_param_f( slaveAxis, PRA_GEAR_ENGAGE_RATE, 1000.0 ); //Gear engage rate = 1000.0 //Gear mode only
		//APS_set_axis_param_f( slaveAxis, PRA_GEAR_RATIO, 1.0 ); //Gear ratio //Gear mode only
		...
		APS_set_axis_param_f( slaveAxis, PRA_GANTRY_PROTECT_2, 10000.0 ); 

		//APS_start_gear( slaveAxis, 1 ); //Set to gear mode
		APS_start_gear( slaveAxis, 2 ); //Set to gantry mode
	}

	// Start a relative p to p move
	APS_ptp( masterAxis, OPT_RELATIVE, 500.0 , 0 );
	Sleep(1000); //wait motion done
